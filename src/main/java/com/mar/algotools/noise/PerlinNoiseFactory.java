...
/**
 * Check:<br/>
 * http://code.google.com/p/fractalterraingeneration/wiki/Perlin_Noise<br/>
 * http://webstaff.itn.liu.se/~stegu/simplexnoise/simplexnoise.pdf<br/>
 * http://gamedev.stackexchange.com/questions/23625/how-do-you-generate-tileable-perlin-noise
 * @author mrenauld
 */
public class PerlinNoiseFactory {
    ...
    private static double[] randCoef;
    ...
    public PerlinNoiseFactory(int pX, int pY) {
        X = pX;
        Y = pY;

        randCoef = new double[20];
        for (int i = 0; i < randCoef.length; ++i) {
            randCoef[i] = Math.random();
        }
    }

    public double[][] getFbmPerlinNoise() {
        double[][] output = new double[X][Y];

        for (int x = 0; x < X; ++x) {
            for (int y = 0; y < Y; ++y) {
                output[x][y] = getFbmPerlinNoiseAtPos(x, y);
            }
        }
     ...
     * @return
     */
    public double getFbmPerlinNoiseAtPos(double pX, double pY) {
        double value = 0.0;
        ...
        // double nX = x1 + Math.sin(ratioX * 1.0 * PI) * dx / (2.0 * PI)
        // * Math.cos(ratioY * 10 * PI * randCoef[0] + randCoef[1]);
        // double nZ = x1 + Math.sin(ratioX * 1.0 * PI) * dx / (2.0 * PI)
        // * Math.cos(ratioY * 10 * PI * randCoef[2] + randCoef[3]);
        //
        // double nY = y1 + Math.sin(ratioY * 1.0 * PI) * dy / (2.0 * PI)
        // * Math.cos(ratioX * 10 * PI * randCoef[4] + randCoef[5]);
        // double nW = y1 + Math.sin(ratioY * 1.0 * PI) * dy / (2.0 * PI)
        // * Math.cos(ratioX * 10 * PI * randCoef[6] + randCoef[7]);

        double nX = x1 + Math.sin(ratioX * 1.0 * PI) * dx / (2.0 * PI) * (ratioY * randCoef[0] + randCoef[1]);
        double nZ = x1 + Math.sin(ratioX * 1.0 * PI) * dx / (2.0 * PI) * (ratioY * randCoef[2] + randCoef[3]);

        double nY = y1 + Math.sin(ratioY * 1.0 * PI) * dy / (2.0 * PI) * (ratioX * randCoef[4] + randCoef[5]);
        double nW = y1 + Math.sin(ratioY * 1.0 * PI) * dy / (2.0 * PI) * (ratioX * randCoef[6] + randCoef[7]);
    ...
    public double[][] getSimplexNoise(double pCoef) {
        double[][] output = new double[X][Y];

        for (int x = 0; x < X; ++x) {
            for (int y = 0; y < Y; ++y) {
                output[x][y] = getSimplexNoise2D(x / pCoef, y / pCoef);
            }
        }
